...
AProyectil::AProyectil()
{
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> ProjectileMeshAsset1(TEXT("StaticMesh'/Game/StarterContent/Shapes/Shape_QuadPyramid.Shape_QuadPyramid'"));

	
	// Create mesh component for the projectile sphere
	MeshBala = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("Proyectilmesh0"));
	MeshBala->SetStaticMesh(ProjectileMeshAsset1.Object);
...
void AProyectil::ChangeMesh(USkeletalMesh* NewMesh)
{
	if (MeshBala && NewMesh)
	{
		// Asigna la nueva malla al componente de malla estática
		//MeshBala->SetSkeletalMesh(NewMesh); // Usa SetStaticMesh si estás usando mallas estáticas
	}
